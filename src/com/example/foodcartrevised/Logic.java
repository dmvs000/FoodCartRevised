package com.example.foodcartrevised;

public class Logic {

	public static String[] restaurents=new String[20];
	public static int rescount=1;
	public static String[][] items=new String[20][20];
	public static String[][] qty=new String[20][20];
	public static int[][] price=new int[20][20];
	public static int totalprice=0;
	public static int itemcount=0;
	
	public static String selectedres="";
	public static String username="";
	public static String unique="";
	public static String ressel="";
	public static String regid="";
	public static String[][] myitem=new String[20][20];
	public static int myrestaurantcount=0;
	public static int myresitemcount=0;
	public static int myresc=0;
	public static int[] myitemc=new int[20];
	
}
